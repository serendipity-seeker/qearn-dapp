...
const ConnectModal = ({ open, onClose }: { open: boolean, onClose: () => void }) => {
    ...
    const selectAccount = () => {
        ...
        const pkSeed = vault.revealSeed(
            accounts[parseInt(selectedAccount.toString())]?.publicId
        )
        connect({
    ...
    return (<>
        ...
        >
            <Card className="relative p-8 w-full max-w-md m-auto flex-col flex"
                ...
                {selectedMode === 'metamask' &&
                    <div className="text-[rgba(128,139,155,1)] mt-4">
                        ...
                        <div className="flex flex-col gap-2 mt-5">
                            <HeaderButtons
                              state={state}
                              onConnectClick={() => mmSnapConnect()}
                            />
                            <button
                              className="bg-[rgba(26,222,245,0.1)] p-3 rounded-lg text-primary-40"
                              onClick={() => setSelectedMode('none')}>Cancel</button>
                        </div>
                    </div>
                }
